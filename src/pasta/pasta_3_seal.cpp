...
namespace pasta
{
  ...
  std::vector<Ciphertext> PASTA_SEAL::encrypt_key_2(std::vector<uint64_t> ssk, bool batch_encoder)
  {                      // DK changes
    (void)batch_encoder; // patched implementation: ignore param
  ...
  std::vector<Ciphertext> PASTA_SEAL::decomposition(
      // Modified from the PASTA_SEAL::HE_decrypt function in
      // https://github.com/IAIK/hybrid-HE-framework/blob/master/ciphers/pasta_3/seal/pasta_3_seal.cpp
  ...
  void PASTA_SEAL::sbox_feistel(seal::Ciphertext &state)
  {
    ...
    for (uint16_t i = PASTA_T; i < halfslots; i++)
      mask_vec[i] = 0;
